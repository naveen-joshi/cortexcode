    ...
    try {
        if (fs.existsSync(indexPath)) {
            ...
            for (const [filePath, fileData] of Object.entries(files)) {
                const symbols = Array.isArray(fileData) ? fileData : fileData.symbols || [];
...
async function checkAndInstallCortexCode() {
...
async function runCortexCodeIndex(rootPath) {
    return new Promise((resolve) => {
...
async function activate(context) {
    ...
    if (!hasCortexCode) {
        ...
        const response = await vscode.window.showInformationMessage('CortexCode CLI not found. Install it to enable AI code indexing?', installBtn);
    ...
    const definitionProvider = {
        provideDefinition(document, position, token) {
            ...
            const symbol = getSymbolAtPosition(document, position);
            if (!symbol || !symbol.file || !symbol.line)
                return null;
...
// ─── Chat Helpers ───
function getSourceCode(filePath, line, context = 15) {
    if (!rootPath) {
        return null;
    }
...
function insertAsCopilotContext(stream, filePath, line, name) {
...
// ─── Chat Handlers ───
function handleSearch(query, stream) {
...
function handleContext(query, stream) {
    if (!indexData) {
        return {};
    }
    const queryLower = query.toLowerCase();
    ...
    for (const [name, syms] of symbolMap.entries()) {
        for (const sym of syms) {
            ...
            else if (sym.file.toLowerCase().includes(queryLower)) {
                score = 3;
            }
    ...
    for (const { sym, score } of top.slice(0, 5)) {
        ...
        if (sym.framework) {
            stream.markdown(`- **Framework:** \`${sym.framework}\`\n`);
        }
        if (sym.params && sym.params.length) {
            stream.markdown(`- **Params:** \`${sym.params.join(', ')}\`\n`);
        }
        if (sym.calls && sym.calls.length) {
            stream.markdown(`- **Calls:** ${sym.calls.slice(0, 8).map(c => `\`${c}\``).join(', ')}\n`);
        }
        // Add source code
        insertAsCopilotContext(stream, sym.file, sym.line, sym.name);
    }
...
function handleImpact(symbolName, stream) {
    if (!indexData) {
        return {};
    }
    const callGraph = indexData.call_graph || {};
...
function handleDeadCode(stream) {
    if (!indexData) {
        return {};
    }
    const callGraph = indexData.call_graph || {};
    ...
    for (const [name, syms] of symbolMap.entries()) {
        ...
        if (['main', 'app', 'init', 'setup', 'handler', 'default', 'index'].includes(name.toLowerCase())) {
            continue;
        }
        if (name.startsWith('__') && name.endsWith('__')) {
            continue;
        }
        if (sym.file.includes('test') || sym.file.includes('spec')) {
            continue;
        }
        dead.push(sym);
    }
...
function handleComplexity(stream) {
    if (!indexData) {
        return {};
    }
    const funcs = [];
